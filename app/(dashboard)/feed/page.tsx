...
export default function FeedPage() {
    ...
    useEffect(() => {
        setLoading(true);
        const endpoint = activeTab === "recommended" ? "/api/events/recommended" : "/api/events";
                    ...
                    <div className="text-center py-20 bg-muted/20 rounded-lg border border-dashed">
                        ...
                        <p className="text-muted-foreground mt-2">
                            {activeTab === "recommended"
                                ? "No events match your profile yet. Check 'All Events' or update your interests."
                            ...
                            <FadeIn key={event._id} className={`delay-[${index * 100}ms]`}>
                                <Card className="h-full flex flex-col overflow-hidden hover:border-primary/50 transition-colors group">
                                    <div className="aspect-video bg-muted relative overflow-hidden">
                                        ...
                                        {(event.allowedCourses?.length > 0 || event.allowedDepartments?.length > 0) && (
                                            <Badge className="absolute bottom-2 right-2 bg-black/50 hover:bg-black/70 text-xs">
                                                Targeted
                                            </Badge>
                                        )}
                                    </div>
